...
class _Target:
    ...
    def __init__(
        self, offset: int, file: Union[str, None], name: str, type: str
    ) -> None:
        self.offset = offset
        self.file = file
        self.name = name
...
class FileAnalyzer(sublime_plugin.ViewEventListener):
    ...
    def on_post_save_async(self):
        if self.__wm.project.is_running:
            asyncio.run_coroutine_threadsafe(
                self.__wm.project.hot_reload(is_manual=False), self.__wm.event_loop
            )

    def __run_target(self, target: _Target):
        if not (w := self.view.window()):
            return

        if target.type == "main" and (file := target.file):
            if (
                os.path.join("lib", "main.dart") in file
                and self.__wm.project.is_flutter_project
            ):
                w.run_command("flutter_run")
            else:
                w.run_command("dart_run")
        else:
            self.__wm.project.run_tests(target.name)
    ...
    def __find_targets(self, path: str):
        ...
        if parts[0] == "test":
            p = subprocess.run(
                [
                    self.__wm.env.dart_path,
                    "run",
                    self.__wm.env.ast_analyzer_path,
                    "tests",
                    path,
                ],
                capture_output=True,
